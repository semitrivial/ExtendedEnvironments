...
What would you do if you were being paid \$15 per hour to act as if you were being
paid \$1,000 per hour? What if, in order to receive that \$15 every hour, you were
required to act \emph{exactly} as you would act if you were really being paid
\$1,000 per hour---and if you did not act exactly as if you were really being paid
\$1,000 per hour, then instead of being paid \$15 that hour, you were penalized
\$15 instead?
This thought experiment would be hard to perform, because it would be hard for your
employer to know how you would act if you really were being paid \$1,000 per hour.
But if your employer could simulate a perfect copy of you, then your employer could
perform the experiment perfectly. Over a long enough time, would those \$15 rewards
and penalties eventually cause you to truly believe you were really being paid
\$1,000 per hour?
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item
        If $e$ is a merciful environment and $h$ is a handicap,
...
pain caused by $h$, the agent otherwise perceives $e$ unaltered (that is, the observations
from $e$ are not changed).
...
\begin{example}
    ...
    version $e'=e*h$ of $e$ modified to incentivize the corresponding type of self-reflection.
    \begin{itemize}
        \item
        ...
        (at pain of punishment) to act as if
        ...
        takes the prescribed actions, the true agent enjoys whatever rewards
        $e$ would give for those actions. The agent can strategically guide the
...
we will finally make some nontrivial usage of observations.
...
\section{Measuring self-reflection}

As an application, self-reflection-incentivizing extended environments could be used
to try and quantify the extent to which different RL agents are self-aware or capable
...
In order to concretely measure the self-reflection of a specific agent (whose source-code
is available to us), we would simply simulate the agent's interaction with some battery
of extended environments that incentivize self-reflection, and see how the agent does.
This approach is crude, because an agent can always perform poorly (resp.\ well)
in any finite set of environments just by dumb luck, despite being quite
...
\cite{chalmers}, one could twist that program into an RL agent (by means of template
programming) and see how well said RL agent performs in extended environments that
incentivize self-reflection.

It might even be possible to use extended environments to test the self-reflection
...
species being driven entirely by ``nurture'' is that any two members of that species,
if raised through identical life circumstances, would act similarly in identical environments.
Thus, if a species were fully nurture-driven, and if two specimens were born in identical
rooms, and experienced identical or near-identical lives,
