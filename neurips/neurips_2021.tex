...
\label{rewardagentforignoringrewardsexample}
  ...
  Choose some fixed observation $o_0$.
...
takes an action $y_n$, the environment simulates the agent in order to determine:
would the agent have taken the same action if the history so far were identical
except for all rewards being $0$? If so, then the environment gives the agent $+1$
reward, otherwise, the environment gives the agent $-1$ reward. Thus, the agent
...
in everyday language. If one merely spoke informally of ``an environment which rewards
