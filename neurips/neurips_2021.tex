...
(RL agents and environments)
  \begin{enumerate}
    \item
    A (traditional) RL environment is a (not necessarily
    deterministic) function which outputs an initial
    perception $x_1$ when given the empty sequence as input;
    and, when given a sequence $x_1y_1\ldots x_ny_n$
    as input (where each $x_i$ is a perception and each
    $y_i$ is an action), outputs a perception $x_{k+1}$.
    \item
    An RL agent is a (not necessarily deterministic)
    function which takes as input a sequence $x_1y_1\ldots x_n$
    and outputs an action $y_n$.
  \end{enumerate}
\end{definition}
