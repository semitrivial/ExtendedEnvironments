...
To show this, two other interpretations are implemented in our open-source library
\cite{library} as IgnoreRewards2.py and IgnoreRewards3.py.
...
\begin{proof}[Proof of Proposition \ref{transformationproposition}]
  ...
  Let $x_1y_1\ldots x_n\in D$
  We will prove, by induction on the length of $s$, that for all $s\in D$,
  $\rho(s)=\pi_{\RC}(s)$.
  The base case is trivial:
  $\rho(\langle x_1\rangle)=\pi(\langle x_1\rangle)=\pi_{\RC}(\langle x_1\rangle)$
  by definition of $\rho$ and $\pi_{\RC}$. For the induction step, assume $s$ has length $>1$,
  so we can write $s=x_1y_1\ldots x_n$, and assume the claim holds for all shorted sequences
  in $D$.
  ...
  $y_i=\rho(x_1y_1\ldots x_i)$. Our induction hypothesis says that for each such $i$,
  $\rho(x_1y_1\ldots x_i)=\pi_{\RC}(x_1y_1\ldots x_i)$,
  so for all $1\leq i<n$, $y_i=\pi_{\RC}(x_1y_1\ldots x_i)$.
  ...
  Thus, for all $1\leq j<i$, $y_j=\pi(x_1y_1\ldots x_i)$.
  ...
  So, every time we check whether or not each $y_i=\pi_{\RC}(x_1y_1\ldots x_n)$ for
  all $1\leq i<n$, we get the same answer.
  If that answer is ``yes'', then we finally output $\pi(x_1y_1\ldots x_n)$ (which is
  determinate since $\pi$ is determinate). Otherwise, we finally output
  $\pi(\langle x_1\rangle)$ (which is determinate since $\pi$ is determinate).
  ...
  For the induction step, assume $n>1$ and that the claim holds for all smaller sequences.
  ...
  =\pi(\langle x_1\rangle)$ as desired.
  ...
  First, the base case, $x_1=x'_1=\mu(\langle\rangle)$ (the environment's initial percept
