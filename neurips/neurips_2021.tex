...
Whenever a set $\O$ of available observations is clear from context,
by a \emph{perception} we mean a pair $(r,o)$ where $o\in\O$ is an
observation and $r\in\mathbb Q$ is a reward.
...
a recurrent DQN agent with lookback 3 (implemented in our library as custom\_DQN.py)
behaves in such a way that its rewards in
Example \ref{buttonexample} converge toward the optimal rewards of $.5$ per turn.
This is fascinating because DQN was not designed with extended environments in mind.
...
\begin{itemize}
  ...
  \item A2C, DQN, and PPO agents from the MIT-licensed open-source Stable Baselines3
    library, converted into agents as in Definition \ref{agentenvironmentdefn}
    using the technique from Section \ref{mappingtouniversalsection}. These agents
    are memoized to make them deterministic. See SBL3\_agents.py in \cite{library}
    for details about how we configured these agents.
  \item RecurrentDQN: A custom implementation of a recurrent DQN agent
    (using standard default hyperparameters).
...
\begin{enumerate}
  ...
  \item Did you specify all the training details (e.g., data splits, hyperparameters, how they were chosen)?
    ...
    the $\log_2$ function. In Section \ref{measurementssection} we referred the
    user to SBL3\_agents.py in \cite{library} for details about how the Stable Baselines3
    agents were configured, and we mentioned that the custom DQN agent was designed
    using industry-standard default hyperparemeters.}
