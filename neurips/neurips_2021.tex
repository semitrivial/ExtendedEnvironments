...
First, let $k$ be the largest power of $2$ such that $k<n$.
...
$x_1y_1\ldots x_{64}$ ($64$ being the largest power of $2$ below $70$),
...
training would be needed until $\pi(x_1y_1\ldots x_{129})$.
...
\end{itemize}
We remark that because of our universalist approach to RL (with no notion of
pre-training the agent for ``a single function in isolation'' \cite{thrun1998lifelong}),
it was necessary to adapt the A2C, DQN, and PPO implementations accordingly.
...
$\pi(x_1y_1\ldots x_n)$, we instantiate an OpenAI Gym-conformant dummy environment
...
for full details about these agents, see \cite{library}.
